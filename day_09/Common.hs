...
import Data.IntMap (IntMap, insert, findWithDefault)
...
getArgValue :: (OperandMode, Int) -> State -> Int
getArgValue i@(m, x) s
    | m == Immediate = x
...
getArgPosition :: (OperandMode, Int) -> State -> Int
...
runProgram s@State{tape=x, pc=c, input=i, output=o, base=b}
    | op == Exit = s
    | otherwise = runProgram State{tape=tape, pc=pc, input=input, output=output, base=base}
    where 
